import { test, expect } from "./fixtures.js";

test.describe("Context Menu", () => {
  test.describe("Visibility", () => {
    test("should show context menu on right-click while active", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should not show context menu when inactive", async ({
      uiGrab,
    }) => {
      const isVisibleBefore = await uiGrab.isOverlayVisible();
      expect(isVisibleBefore).toBe(false);

      await uiGrab.rightClickElement("li");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(false);
    });

    test("should show context menu after keyboard activation", async ({
      uiGrab,
    }) => {
      await uiGrab.activateViaKeyboard();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should show context menu when right-clicking while holding activation keys", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();

      await uiGrab.page.keyboard.down(uiGrab.modifierKey);
      await uiGrab.page.keyboard.down("c");
      await uiGrab.page.waitForTimeout(100);

      const element = uiGrab.page.locator("li").first();
      await element.click({ button: "right", force: true });

      await expect
        .poll(() => uiGrab.isContextMenuVisible(), { timeout: 2000 })
        .toBe(true);

      await uiGrab.page.keyboard.up("c");
      await uiGrab.page.keyboard.up(uiGrab.modifierKey);
    });

    test("should show context menu with Copy and Open items", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);

      const isCopyEnabled = await uiGrab.isContextMenuItemEnabled("Copy");
      expect(isCopyEnabled).toBe(true);

      const isOpenEnabled = await uiGrab.isContextMenuItemEnabled("Open");
      expect(isOpenEnabled).toBe(true);
    });
  });

  test.describe("Menu Items", () => {
    test("should copy element when clicking Copy", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("[data-testid='todo-list'] h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("[data-testid='todo-list'] h1");
      await uiGrab.clickContextMenuItem("Copy");

      await uiGrab.page.waitForTimeout(500);

      const clipboardContent = await uiGrab.getClipboardContent();
      expect(clipboardContent).toContain("Todo List");
    });

    test("should copy list item content correctly", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.clickContextMenuItem("Copy");

      await uiGrab.page.waitForTimeout(500);

      const clipboardContent = await uiGrab.getClipboardContent();
      expect(clipboardContent).toBeTruthy();
    });

    test("should have Copy always enabled", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isCopyEnabled = await uiGrab.isContextMenuItemEnabled("Copy");
      expect(isCopyEnabled).toBe(true);
    });
  });

  test.describe("Dismissal", () => {
    test("should dismiss context menu on Escape", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isVisibleBefore = await uiGrab.isContextMenuVisible();
      expect(isVisibleBefore).toBe(true);

      await uiGrab.page.keyboard.press("Escape");
      await uiGrab.page.waitForTimeout(200);

      const isVisibleAfter = await uiGrab.isContextMenuVisible();
      expect(isVisibleAfter).toBe(false);
    });

    test("should dismiss context menu when clicking outside", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isVisibleBefore = await uiGrab.isContextMenuVisible();
      expect(isVisibleBefore).toBe(true);

      await uiGrab.page.mouse.click(10, 10);
      await uiGrab.page.waitForTimeout(200);

      const isVisibleAfter = await uiGrab.isContextMenuVisible();
      expect(isVisibleAfter).toBe(false);
    });

    test("should dismiss context menu after Copy action", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      await uiGrab.clickContextMenuItem("Copy");
      await uiGrab.page.waitForTimeout(300);

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(false);
    });
  });

  test.describe("Selection Freezing", () => {
    test("should freeze element selection while context menu is open", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);

      await uiGrab.page.mouse.move(100, 100);
      await uiGrab.page.waitForTimeout(100);

      const stillVisible = await uiGrab.isContextMenuVisible();
      expect(stillVisible).toBe(true);
    });

    test("should maintain context menu while moving mouse", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");

      await uiGrab.page.mouse.move(500, 500);
      await uiGrab.page.waitForTimeout(100);

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });
  });

  test.describe("Multiple Context Menus", () => {
    test("should allow opening new context menu after using previous one", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");
      await uiGrab.clickContextMenuItem("Copy");

      await uiGrab.page.waitForTimeout(300);

      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should allow opening context menu after clicking outside to dismiss", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");

      await uiGrab.page.mouse.click(10, 10);
      await uiGrab.page.waitForTimeout(300);

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");

      await expect
        .poll(async () => uiGrab.isSelectionBoxVisible(), { timeout: 5000 })
        .toBe(true);

      await uiGrab.rightClickElement("li:first-child");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should show context menu on different elements consecutively", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();

      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");
      const firstMenuVisible = await uiGrab.isContextMenuVisible();
      expect(firstMenuVisible).toBe(true);

      await uiGrab.page.mouse.click(10, 10);
      await uiGrab.page.waitForTimeout(200);

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const secondMenuVisible = await uiGrab.isContextMenuVisible();
      expect(secondMenuVisible).toBe(true);
    });

    test("should keep context menu on original element when right-clicking different element while menu is open", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();

      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");
      const firstMenuVisible = await uiGrab.isContextMenuVisible();
      expect(firstMenuVisible).toBe(true);

      // Right-clicking elsewhere while menu is open should NOT switch to new element
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.page.waitForTimeout(100);

      const menuStillVisible = await uiGrab.isContextMenuVisible();
      expect(menuStillVisible).toBe(true);
    });
  });

  test.describe("Keyboard Navigation Integration", () => {
    test("should show context menu after keyboard navigation", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();

      await uiGrab.pressArrowDown();
      await uiGrab.waitForSelectionBox();

      await uiGrab.rightClickElement("li:nth-child(2)");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should copy correct element after keyboard navigation via context menu", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();

      await uiGrab.pressArrowDown();
      await uiGrab.page.waitForTimeout(100);
      await uiGrab.waitForSelectionBox();

      await uiGrab.rightClickElement("li:nth-child(2)");
      await uiGrab.clickContextMenuItem("Copy");

      await uiGrab.page.waitForTimeout(500);

      const clipboardContent = await uiGrab.getClipboardContent();
      expect(clipboardContent).toContain("Walk the dog");
    });
  });

  test.describe("Element-specific Behavior", () => {
    test("should show context menu for heading element", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should show context menu for list element", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("ul");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("ul");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should show context menu for list item element", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li:nth-child(2)");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:nth-child(2)");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });
  });

  test.describe("Edge Cases", () => {
    test("should work correctly after scrolling page", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.scrollPage(100);
      await uiGrab.page.waitForTimeout(100);

      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should allow reopening after dismiss and copy flow", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li");
      await uiGrab.clickContextMenuItem("Copy");

      await uiGrab.page.waitForTimeout(500);

      await uiGrab.activate();
      await uiGrab.hoverElement("h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("h1");

      const isContextMenuVisible = await uiGrab.isContextMenuVisible();
      expect(isContextMenuVisible).toBe(true);
    });

    test("should copy different elements via context menu", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("[data-testid='todo-list'] h1");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("[data-testid='todo-list'] h1");
      await uiGrab.clickContextMenuItem("Copy");
      await uiGrab.page.waitForTimeout(1600);

      const firstCopy = await uiGrab.getClipboardContent();
      expect(firstCopy).toContain("Todo List");

      await uiGrab.activate();
      await uiGrab.page.waitForTimeout(100);
      await uiGrab.hoverElement("[data-testid='todo-list'] li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement(
        "[data-testid='todo-list'] li:first-child",
      );
      await uiGrab.clickContextMenuItem("Copy");
      await uiGrab.page.waitForTimeout(500);

      const secondCopy = await uiGrab.getClipboardContent();
      expect(secondCopy).toBeTruthy();
      expect(secondCopy).not.toContain("Todo List");
    });
  });

  test.describe("Prompt Menu Item", () => {
    test("Edit item should appear when agent is configured", async ({
      uiGrab,
    }) => {
      await uiGrab.setupMockAgent();
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const menuInfo = await uiGrab.getContextMenuInfo();
      expect(menuInfo.isVisible).toBe(true);
      expect(menuInfo.menuItems).toContain("Edit");
    });

    test("Edit item should enter input mode when clicked", async ({
      uiGrab,
    }) => {
      await uiGrab.setupMockAgent();
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.page.waitForTimeout(100);

      await uiGrab.clickContextMenuItem("Edit");
      await uiGrab.page.waitForTimeout(200);

      const isPromptMode = await uiGrab.isPromptModeActive();
      expect(isPromptMode).toBe(true);
    });

    test("Edit with agent keeps overlay active", async ({ uiGrab }) => {
      await uiGrab.setupMockAgent();
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.clickContextMenuItem("Edit");

      await expect
        .poll(() => uiGrab.isOverlayVisible(), { timeout: 2000 })
        .toBe(true);
    });

    test("Copy without agent deactivates after action", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.clickContextMenuItem("Copy");

      await expect
        .poll(() => uiGrab.isOverlayVisible(), { timeout: 3000 })
        .toBe(false);
    });
  });

  test.describe("Context Menu Positioning", () => {
    test("context menu should appear near click position", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("[data-testid='todo-list'] li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement(
        "[data-testid='todo-list'] li:first-child",
      );
      await uiGrab.page.waitForTimeout(200);

      const menuInfo = await uiGrab.getContextMenuInfo();
      expect(menuInfo.isVisible).toBe(true);
      expect(menuInfo.position).toBeDefined();
    });

    test("context menu should stay within viewport at bottom edge", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("[data-testid='edge-bottom-left']");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("[data-testid='edge-bottom-left']");

      const menuInfo = await uiGrab.getContextMenuInfo();
      const viewport = await uiGrab.getViewportSize();

      if (menuInfo.position) {
        expect(menuInfo.position.y).toBeLessThan(viewport.height);
      }
    });

    test("context menu should stay within viewport at right edge", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("[data-testid='edge-top-right']");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("[data-testid='edge-top-right']");

      const menuInfo = await uiGrab.getContextMenuInfo();
      const viewport = await uiGrab.getViewportSize();

      if (menuInfo.position) {
        expect(menuInfo.position.x).toBeLessThan(viewport.width);
      }
    });
  });

  test.describe("Custom Actions with Agent", () => {
    test("custom action with agent should appear in menu", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        const mockProvider = {
          *send() {
            yield "Processing...";
            yield "Completed";
          },
          supportsFollowUp: true,
        };

        api?.unregisterPlugin("custom-agent-action");
        const agent = { provider: mockProvider };
        api?.registerPlugin({
          name: "custom-agent-action",
          actions: [
            {
              id: "custom-edit",
              label: "Custom Edit",
              shortcut: "E",
              onAction: (context: {
                enterPromptMode?: (agent?: Record<string, unknown>) => void;
              }) => {
                context.enterPromptMode?.(agent);
              },
              agent,
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const menuInfo = await uiGrab.getContextMenuInfo();
      expect(menuInfo.isVisible).toBe(true);
      expect(
        menuInfo.menuItems.map((item: string) => item.toLowerCase()),
      ).toContain("custom edit");
    });

    test("custom action should trigger enterPromptMode", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        const mockProvider = {
          *send() {
            yield "Processing...";
            yield "Completed";
          },
          supportsFollowUp: true,
        };

        api?.unregisterPlugin("custom-agent-action");
        const agent = { provider: mockProvider };
        api?.registerPlugin({
          name: "custom-agent-action",
          actions: [
            {
              id: "custom-edit",
              label: "Custom Edit",
              shortcut: "E",
              onAction: (context: {
                enterPromptMode?: (agent?: Record<string, unknown>) => void;
              }) => {
                context.enterPromptMode?.(agent);
              },
              agent,
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.page.waitForTimeout(100);

      await uiGrab.clickContextMenuItem("Custom edit");
      await uiGrab.page.waitForTimeout(200);

      const isPromptMode = await uiGrab.isPromptModeActive();
      expect(isPromptMode).toBe(true);
    });

    test("action without agent should just execute onAction", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        api?.unregisterPlugin("plain-action");
        api?.registerPlugin({
          name: "plain-action",
          actions: [
            {
              id: "plain-action",
              label: "Plain Action",
              onAction: () => {
                (
                  window as { __plainActionCalled?: boolean }
                ).__plainActionCalled = true;
              },
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const menuInfo = await uiGrab.getContextMenuInfo();
      const lowerMenuItems = menuInfo.menuItems.map((item: string) =>
        item.toLowerCase(),
      );
      expect(lowerMenuItems).toContain("plain action");

      await uiGrab.clickContextMenuItem("Plain Action");
      await uiGrab.page.waitForTimeout(100);

      const actionCalled = await uiGrab.page.evaluate(
        () =>
          (window as { __plainActionCalled?: boolean }).__plainActionCalled ??
          false,
      );
      expect(actionCalled).toBe(true);
    });

    test("multiple actions should all appear in menu", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        api?.unregisterPlugin("multi-actions");
        api?.registerPlugin({
          name: "multi-actions",
          actions: [
            {
              id: "action-1",
              label: "First Action",
              onAction: () => {},
            },
            {
              id: "action-2",
              label: "Second Action",
              onAction: () => {},
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const menuInfo = await uiGrab.getContextMenuInfo();
      const lowerMenuItems = menuInfo.menuItems.map((item: string) =>
        item.toLowerCase(),
      );
      expect(lowerMenuItems).toContain("first action");
      expect(lowerMenuItems).toContain("second action");
    });

    test("action with shortcut should be triggerable via keyboard", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        api?.unregisterPlugin("keyboard-action");
        api?.registerPlugin({
          name: "keyboard-action",
          actions: [
            {
              id: "keyboard-action",
              label: "Keyboard Action",
              shortcut: "K",
              onAction: () => {
                (
                  window as { __keyboardActionCalled?: boolean }
                ).__keyboardActionCalled = true;
              },
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");
      await uiGrab.page.waitForTimeout(100);

      await uiGrab.pressModifierKeyCombo("k");
      await uiGrab.page.waitForTimeout(200);

      const actionCalled = await uiGrab.page.evaluate(
        () =>
          (window as { __keyboardActionCalled?: boolean })
            .__keyboardActionCalled ?? false,
      );
      expect(actionCalled).toBe(true);
    });

    test("disabled action should appear but be disabled", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        api?.unregisterPlugin("disabled-action");
        api?.registerPlugin({
          name: "disabled-action",
          actions: [
            {
              id: "disabled-action",
              label: "Disabled Action",
              enabled: false,
              onAction: () => {},
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const isDisabled = await uiGrab.page.evaluate((attrName) => {
        const host = document.querySelector(`[${attrName}]`);
        const shadowRoot = host?.shadowRoot;
        if (!shadowRoot) return false;
        const root = shadowRoot.querySelector(`[${attrName}]`);
        const button = root?.querySelector(
          '[data-ui-grab-menu-item="disabled action"]',
        );
        return button?.hasAttribute("disabled") ?? false;
      }, "data-ui-grab");
      expect(isDisabled).toBe(true);
    });

    test("action enabled function should receive context", async ({
      uiGrab,
    }) => {
      await uiGrab.page.evaluate(() => {
        const api = (
          window as {
            __UI_GRAB__?: {
              unregisterPlugin: (name: string) => void;
              registerPlugin: (plugin: Record<string, unknown>) => void;
            };
          }
        ).__UI_GRAB__;

        api?.unregisterPlugin("context-action");
        api?.registerPlugin({
          name: "context-action",
          actions: [
            {
              id: "context-action",
              label: "Context Action",
              enabled: (context: { element: Element }) => {
                (window as { __enabledTagName?: string }).__enabledTagName =
                  context.element.tagName;
                return context.element.tagName.toLowerCase() === "li";
              },
              onAction: () => {},
            },
          ],
        });
      });

      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.rightClickElement("li:first-child");

      const enabledTagName = await uiGrab.page.evaluate(
        () => (window as { __enabledTagName?: string }).__enabledTagName,
      );
      expect(enabledTagName).toBe("LI");

      const menuInfo = await uiGrab.getContextMenuInfo();
      const lowerMenuItems = menuInfo.menuItems.map((item: string) =>
        item.toLowerCase(),
      );
      expect(lowerMenuItems).toContain("context action");
    });
  });
});
