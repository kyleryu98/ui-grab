...
export interface UiGrabPageObject {
  ...
  dragToolbarResizeHandle: (deltaX: number, deltaY: number) => Promise<void>;
  ...
  getCommentItemCount: () => Promise<number>;
  clickCommentItem: (index: number) => Promise<void>;
  clickCommentItemCopy: (index: number) => Promise<void>;
  clickCommentItemDelete: (index: number) => Promise<void>;
...
const createUiGrabPageObject = (page: Page): UiGrabPageObject => {
  ...
  const isOverlayVisible = async () => {
    return page.evaluate(() => {
      const api = (window as { __UI_GRAB__?: { isActive: () => boolean } })
        .__UI_GRAB__;
  ...
  const waitForSelectionTarget = async (timeout = 10_000) => {
    await page.waitForFunction(
      () => {
        const api = (
          window as {
  ...
  const activate = async () => {
    await page.evaluate(() => {
      const api = (window as { __UI_GRAB__?: { activate: () => void } })
        .__UI_GRAB__;
      api?.activate();
    });
  ...
  const hoverElement = async (selector: string) => {
    ...
    try {
      await waitForSelectionTarget(400);
  ...
  const waitForContextMenu = async (visible: boolean) => {
    await page.waitForFunction(
      ({ attrName, expectedVisible }) => {
        ...
        const menuItem = root.querySelector("[data-ui-grab-menu-item]");
    ...
    await page.evaluate(
      ({ attrName, itemLabel }) => {
        ...
        const button = root.querySelector<HTMLButtonElement>(
          `[data-ui-grab-menu-item="${itemLabel.toLowerCase()}"]`,
        );
        if (!button)
    ...
    return page.evaluate((attrName) => {
      ...
      const menuItemButtons = Array.from(
        contextMenu.querySelectorAll<HTMLButtonElement>(
          "[data-ui-grab-menu-item]",
        ),
      );
      const menuItems = menuItemButtons.map((btn) => {
        const item = btn.dataset.uiGrabMenuItem ?? "";
  ...
  const waitForPromptMode = async (active: boolean) => {
    await page.waitForFunction(
      (expected) => {
        const api = (
          window as {
            __UI_GRAB__?: { getState: () => { isPromptMode: boolean } };
          }
        ).__UI_GRAB__;
  ...
  const getInputValue = async (): Promise<string> => {
    return page.evaluate((attrName) => {
      ...
      const textarea = root.querySelector(
        "textarea[data-ui-grab-ignore-events]",
  ...
  const isPendingDismissVisible = async (): Promise<boolean> => {
    return page.evaluate((attrName) => {
      ...
      const root = shadowRoot.querySelector(`[${attrName}]`);
      if (!root) return false;
      const discardPrompt = root.querySelector("[data-ui-grab-discard-prompt]");
  ...
  const clickCommentsButton = async () => {
    ...
    await clickShadowRootButton("[data-ui-grab-toolbar-comments]");
  ...
  const isCommentsDropdownVisible = async (): Promise<boolean> => {
    return page.evaluate((attrName) => {
      ...
      const root = shadowRoot.querySelector(`[${attrName}]`);
      if (!root) return false;
      const dropdown = root.querySelector("[data-ui-grab-comments-dropdown]");
  ...
  const getCommentsDropdownInfo = async (): Promise<CommentsDropdownInfo> => {
    return page.evaluate((attrName) => {
      ...
      return {
        isVisible: true,
        itemCount: dropdown.querySelectorAll("[data-ui-grab-comment-item]")
          .length,
      };
    }, ATTRIBUTE_NAME);
  };

  const getCommentItemCount = async (): Promise<number> => {
    return page.evaluate(() => {
      try {
  ...
  const clickCommentItemAction = async (
    index: number,
    actionSelector: string,
  ) => {
    await page.evaluate(
      ({ attrName, itemIndex, selector }) => {
        const host = document.querySelector(`[${attrName}]`);
        const shadowRoot = host?.shadowRoot;
        if (!shadowRoot) return;
        const root = shadowRoot.querySelector(`[${attrName}]`);
        ...
        const item = items[itemIndex];
        if (!item) return;
        const action = item.querySelector<HTMLButtonElement>(selector);
        action?.click();
      },
      { attrName: ATTRIBUTE_NAME, itemIndex: index, selector: actionSelector },
    );
  };

  const clickCommentItemCopy = async (index: number) => {
    await clickCommentItemAction(index, "[data-ui-grab-comment-copy]");
  };

  const clickCommentItemDelete = async (index: number) => {
    await clickCommentItemAction(index, "[data-ui-grab-comment-delete]");
  };

  const clickCommentsCopyAll = async () => {
    await clickShadowRootButton("[data-ui-grab-comments-copy-all]");
  };

  const clickCommentsClear = async () => {
    await clickShadowRootButton("[data-ui-grab-comments-clear]");
  ...
  const hoverCommentItem = async (index: number) => {
    const itemRect = await page.evaluate(
      ({ attrName, itemIndex }) => {
        ...
        const root = shadowRoot.querySelector(`[${attrName}]`);
        if (!root) return null;
        const items = root.querySelectorAll("[data-ui-grab-comment-item]");
  ...
  const hoverCommentsButton = async () => {
    const waitForCommentsHoverPreview = async (timeout = 600) => {
      await page.waitForFunction(
        (attrName) => {
          const api = (
            window as {
          ...
          const grabbedBoxes = api?.getState()?.grabbedBoxes ?? [];
          if (
            grabbedBoxes.some((box) => box.id.startsWith("comment-all-hover-"))
          ) {
            return true;
          }
    ...
    if (buttonRect) {
      ...
      const attempts = [
        { x: Math.max(buttonRect.x - 48, 0), y: centerY },
        { x: centerX, y: Math.max(buttonRect.y - 48, 0) },
        {
          x: Math.max(buttonRect.x - 48, 0),
          y: Math.max(buttonRect.y - 48, 0),
        },
      ];

      for (const attempt of attempts) {
        await page.mouse.move(attempt.x, attempt.y);
        await page.waitForTimeout(50);
        await page.mouse.move(centerX, centerY, { steps: 6 });
        try {
          await waitForCommentsHoverPreview();
          await page.waitForTimeout(100);
          return;
        } catch {
          // Retry from a different outside approach point.
        }
      }
  ...
  const clickToolbarCopyAll = async () => {
    await clickShadowRootButton("[data-ui-grab-toolbar-copy-all]");
  };
  ...
  const isClearCommentsPromptVisible = async (): Promise<boolean> => {
    return page.evaluate((attrName) => {
      ...
      const prompt = root.querySelector<HTMLElement>(
        "[data-ui-grab-clear-comments-prompt]",
      );
  ...
  const confirmClearCommentsPrompt = async () => {
    await page.evaluate((attrName) => {
      ...
      const root = shadowRoot.querySelector(`[${attrName}]`);
      if (!root) return;
      const prompt = root.querySelector("[data-ui-grab-clear-comments-prompt]");
      if (!prompt) return;
      const yesButton = prompt.querySelector<HTMLButtonElement>(
        "[data-ui-grab-discard-yes]",
      );
      yesButton?.click();
    }, ATTRIBUTE_NAME);
  };
  ...
  const getSelectionLabelInfo = async (): Promise<SelectionLabelInfo> => {
    return page.evaluate((attrName) => {
      ...
      const label = root.querySelector("[data-ui-grab-selection-label]");
      if (!label)
        return {
          isVisible: false,
          tagName: null,
  ...
  const getState = async (): Promise<UiGrabState> => {
    return page.evaluate(() => {
      const api = (window as { __UI_GRAB__?: { getState: () => UiGrabState } })
        .__UI_GRAB__;
      const state = api?.getState();
      return (
        state ?? {
  ...
  const toggle = async () => {
    const wasActive = await isOverlayVisible();
    await page.evaluate(() => {
      const api = (window as { __UI_GRAB__?: { toggle: () => void } })
        .__UI_GRAB__;
      api?.toggle();
    });
    await waitForActive(!wasActive);
  ...
  const dispose = async () => {
    await page.evaluate(() => {
      const api = (window as { __UI_GRAB__?: { dispose: () => void } })
        .__UI_GRAB__;
      api?.dispose();
    });
  };
  ...
  const copyElementViaApi = async (selector: string): Promise<boolean> => {
    return page.evaluate(async (sel) => {
      const api = (
        window as {
          __UI_GRAB__?: { copyElement: (el: Element) => Promise<boolean> };
        }
      ).__UI_GRAB__;
  ...
  const reinitialize = async (options?: Record<string, unknown>) => {
    await page.evaluate((opts) => {
      const existingApi = (window as { __UI_GRAB__?: { dispose: () => void } })
        .__UI_GRAB__;
      existingApi?.dispose();

      const initFn = (
        window as { initUiGrab?: (o?: Record<string, unknown>) => void }
      ).initUiGrab;
      initFn?.(opts);
    }, options);
    await page.waitForFunction(
      () => {
        const api = (window as { __UI_GRAB__?: unknown }).__UI_GRAB__;
    ...
    await page.waitForFunction(
      (attrName) => {
        ...
        const sessionElements = Array.from(
          root.querySelectorAll("[data-ui-grab-ignore-events]"),
        );
  ...
  const isTouchMode = async (): Promise<boolean> => {
    return page.evaluate(() => {
      const api = (
        window as {
          __UI_GRAB__?: { getState: () => { isTouchMode?: boolean } };
        }
      ).__UI_GRAB__;
      return (
  ...
  return {
    ...
    getCommentItemCount,
    clickCommentItem,
    clickCommentItemCopy,
    clickCommentItemDelete,
...
export const test = base.extend<{ uiGrab: UiGrabPageObject }>({
  uiGrab: async ({ page }, use) => {
    ...
    const initializePage = async () => {
      ...
      for (
        ...
        if (page.isClosed()) {
          throw new Error("Browser page closed during uiGrab fixture setup");
        }
        try {
          await page.goto("/", {
    ...
    const uiGrab = createUiGrabPageObject(page);
    await use(uiGrab);
  },
});
