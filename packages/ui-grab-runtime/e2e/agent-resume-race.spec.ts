...
test.describe("Agent Resume Race", () => {
  test("keeps resumed session visible when old cleanup finishes", async ({
    ...
    await uiGrab.page.evaluate(
      ({ oldStreamAbortDelayMs, resumeStatusIntervalMs }) => {
        ...
        const installResumeRaceAgent = (): void => {
          ...
          const api = currentWindow.__UI_GRAB__;
    ...
    await uiGrab.typeInInput("Trigger resume race");
    await uiGrab.submitInput();
    await uiGrab.waitForAgentSession(4000);
    ...
    await uiGrab.page.waitForTimeout(RACE_SETTLE_WAIT_MS);

    await expect
      .poll(() => uiGrab.isAgentSessionVisible(), {
        timeout: 4000,
      })
      .toBe(true);
      ...
      .poll(
        () =>
          uiGrab.page.evaluate(
            ...
            {
              attributeName: "data-ui-grab",
              discardYesSelector: "[data-ui-grab-discard-yes]",
            },
          ),
        { timeout: 2000 },
      )
