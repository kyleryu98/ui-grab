...
test.describe("Event Callbacks", () => {
  test.beforeEach(async ({ uiGrab }) => {
    await uiGrab.setupCallbackTracking();
    await uiGrab.clearCallbackHistory();
  });
  ...
  test.describe("Activation Callbacks", () => {
    test("onActivate should fire when overlay is activated", async ({
      ...
      const args = await uiGrab.waitForCallback("onActivate", 2000);
    ...
    test("onDeactivate should fire when overlay is deactivated", async ({
      ...
      const args = await uiGrab.waitForCallback("onDeactivate", 2000);
      expect(args).toBeDefined();
    });
    ...
    test("onActivate should fire before onDeactivate in activation cycle", async ({
      ...
      const history = await uiGrab.getCallbackHistory();
  ...
  test.describe("Drag Callbacks", () => {
    test("onDragStart should fire when drag begins", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.clearCallbackHistory();
    ...
    test("onDragEnd should fire when drag completes", async ({ uiGrab }) => {
      await uiGrab.activate();
    ...
    test("onDragStart should include coordinates", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.clearCallbackHistory();
  ...
  test.describe("State Change Callback", () => {
    test("onStateChange should fire on activation", async ({ uiGrab }) => {
      await uiGrab.clearCallbackHistory();
    ...
    test("onStateChange should fire on deactivation", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.clearCallbackHistory();
    ...
    test("onStateChange should fire during drag", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.clearCallbackHistory();
  ...
  test.describe("Context Menu Callback", () => {
    test("onContextMenu should fire on right-click", async ({ uiGrab }) => {
      await uiGrab.activate();
      await uiGrab.hoverElement("li:first-child");
      await uiGrab.waitForSelectionBox();
      await uiGrab.clearCallbackHistory();
  ...
  test.describe("Callback Integrity", () => {
    ...
    test("callbacks should include timestamps", async ({ uiGrab }) => {
      await uiGrab.clearCallbackHistory();
    ...
    test("multiple callbacks should maintain order", async ({ uiGrab }) => {
      await uiGrab.clearCallbackHistory();
