...
const navigateAndWaitForUiGrab = async (page: Page): Promise<void> => {
...
const activateViaApi = (page: Page): Promise<void> =>
  page.evaluate(() => {
    (
      window as unknown as { __UI_GRAB__: { activate: () => void } }
    ).__UI_GRAB__.activate();
  });

const deactivateViaApi = (page: Page): Promise<void> =>
  page.evaluate(() => {
    (
      window as unknown as { __UI_GRAB__: { deactivate: () => void } }
    ).__UI_GRAB__.deactivate();
  });
...
test.describe("Freeze Animations", () => {
  test.describe("Page Animation Freezing", () => {
    ...
    test("should not leave page animations in paused state after deactivation", async ({
      ...
      const pausedPageAnimationCount = await uiGrab.page.evaluate(
        (attrName) => {
    ...
    test("should not leave global freeze style element in document after deactivation", async ({
      ...
      const hasFreezeStyleDuring = await uiGrab.page.evaluate(() => {
        return (
          document.querySelector("[data-ui-grab-global-freeze]") !== null
        );
      });
      ...
      const hasFreezeStyleAfter = await uiGrab.page.evaluate(() => {
        return (
          document.querySelector("[data-ui-grab-global-freeze]") !== null
        );
      });
  ...
  test.describe("UI Grab UI Preservation", () => {
    test("should not finish ui-grab shadow DOM animations on deactivation", async ({
      uiGrab,
    }) => {
      await uiGrab.activate();
      ...
      const shadowAnimationCountBefore = await uiGrab.page.evaluate(
        (attrName) => {
          return document.getAnimations().filter((animation) => {
            if (animation.effect instanceof KeyframeEffect) {
              const target = animation.effect.target;
      ...
      const shadowAnimationCountAfter = await uiGrab.page.evaluate(
        (attrName) => {
          return document.getAnimations().filter((animation) => {
            if (animation.effect instanceof KeyframeEffect) {
              const target = animation.effect.target;
  ...
  test.describe("Freeze/Unfreeze Cycles", () => {
    test("should handle rapid activation cycles without breaking animations", async ({
      ...
      const hasFreezeStyle = await uiGrab.page.evaluate(() => {
        return (
          document.querySelector("[data-ui-grab-global-freeze]") !== null
        );
      });
      ...
      const toolbarVisible = await uiGrab.isToolbarVisible();
    ...
    test("should correctly freeze animations after reactivation", async ({
      ...
      const pausedAnimationCount = await uiGrab.page.evaluate((attrName) => {
        return document.getAnimations().filter((animation) => {
          if (animation.effect instanceof KeyframeEffect) {
            const target = animation.effect.target;
            if (target instanceof Element) {
  ...
  test.describe("rAF Interception", () => {
    test("should wrap window.requestAnimationFrame and cancelAnimationFrame", async ({
      ...
      const isWrapped = await uiGrab.page.evaluate(() => {
    ...
    test("should execute non-animation rAF callbacks during freeze", async ({
      ...
      const didCallbackExecute = await uiGrab.page.evaluate(() => {
    ...
    test("should hold animation library callbacks during freeze", async ({
      ...
      await simulateGsapPresence(uiGrab.page);
      await uiGrab.activate();
      await uiGrab.page.waitForTimeout(100);

      const wasCallbackHeld = await uiGrab.page.evaluate(() => {
        return new Promise<boolean>((resolve) => {
    ...
    test("should release held callbacks after unfreeze", async ({
      ...
      await uiGrab.page.waitForTimeout(100);
      const wasHeldDuringFreeze = await uiGrab.page.evaluate(
        () =>
          !(window as unknown as Record<string, boolean>).__GSAP_TEST_FLAG__,
      );
      ...
      const wasReleasedAfterUnfreeze = await uiGrab.page.evaluate(
        () => (window as unknown as Record<string, boolean>).__GSAP_TEST_FLAG__,
      );
    ...
    test("should cancel held callbacks via cancelAnimationFrame", async ({
      ...
      const wasCancelledWhileHeld = await uiGrab.page.evaluate(() => {
        return new Promise<boolean>((resolve) => {
    ...
    test("should cancel held callbacks across evaluate calls via returned id", async ({
      ...
      const heldId = await uiGrab.page.evaluate(() => {
      ...
      await uiGrab.page.evaluate((identifier: number) => {
      ...
      const didCallbackRun = await uiGrab.page.evaluate(
        () =>
          (window as unknown as Record<string, boolean>).__RACE_CANCEL_FLAG__,
      );
    ...
    test("should cancel replayed callbacks via fake id after unfreeze", async ({
      ...
      await navigateAndWaitForUiGrab(page);
    ...
    test("should not intercept callbacks after unfreeze", async ({
      ...
      const didCallbackExecuteNormally = await uiGrab.page.evaluate(() => {
        return new Promise<boolean>((resolve) => {
          // HACK: function named _tick simulates GSAP's internal tick,
          // detected via stack trace inspection in the rAF wrapper
          const _tick = () => {
