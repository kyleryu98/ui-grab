...
const COMMENT_ITEMS_KEY = "ui-grab-comment-items";
const LEGACY_COMMENT_ITEMS_KEY = "ui-grab-history-items";
const CLEAR_CONFIRMED_KEY = "ui-grab-clear-confirmed";
...
export const updateCommentItem = (
  itemId: string,
  updates: Partial<Omit<CommentItem, "id">>,
): CommentItem[] => {
  const existingItem = commentItems.find((item) => item.id === itemId);
  if (!existingItem) return commentItems;
  ...
  return persistCommentItems(
    commentItems.map((item) => (item.id === itemId ? nextItem : item)),
  );
};
