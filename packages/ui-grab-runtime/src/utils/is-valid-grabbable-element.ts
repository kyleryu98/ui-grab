...
const isUiGrabElement = (element: Element): boolean => {
  if (element.hasAttribute("data-ui-grab")) return true;

  const rootNode = element.getRootNode();
  return (
    rootNode instanceof ShadowRoot &&
    rootNode.host.hasAttribute("data-ui-grab")
  );
};
...
// Mirrors the upstream overlay-host exclusion behavior discussed in issue 148.
