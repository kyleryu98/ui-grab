...
export interface Theme {
  ...
  /**
   * The floating toolbar that allows toggling UI Grab activation
   */
  toolbar?: {
    /**
...
export interface PluginHooks {
  ...
  onStateChange?: (state: UiGrabState) => void;
  onPromptModeChange?: (
    isPromptMode: boolean,
...
export interface Plugin {
  ...
  actions?: ContextMenuAction[];
  hooks?: PluginHooks;
  setup?: (api: UiGrabAPI, hooks: ActionContextHooks) => PluginConfig | void;
}
...
export interface Options {
  ...
  /**
   * Whether to freeze React state updates while UI Grab is active.
...
export interface ToolbarState {
  ...
  width?: number;
  height?: number;
  // Legacy long-axis size. Kept for backward compatibility with older state.
  size?: number;
...
export interface UiGrabRendererProps {
