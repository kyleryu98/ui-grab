...
import { toggleElementSelection } from "../utils/toggle-element-selection.js";
...
export const init = (rawOptions?: Options): UiGrabAPI => {
  ...
  return createRoot((dispose) => {
    ...
    const [activeCommentItemId, setActiveCommentItemId] = createSignal<
      string | null
    >(null);
    const [editingCommentItemId, setEditingCommentItemId] = createSignal<
      string | null
    >(null);
    ...
    const updateToolbarState = (updates: Partial<ToolbarState>) => {
      ...
      const edge = updates.edge ?? currentState?.edge ?? "bottom";
      const scale = resolveToolbarScale(
        {
          ...currentState,
          ...updates,
        },
      ...
      const newState: ToolbarState = {
        ...
        ratio:
          ...
          currentState?.ratio ??
          TOOLBAR_DEFAULT_POSITION_RATIO,
        collapsed: updates.collapsed ?? currentState?.collapsed ?? false,
        enabled: updates.enabled ?? currentState?.enabled ?? true,
        scale,
        width: undefined,
        height: undefined,
        size: undefined,
        defaultAction:
          updates.defaultAction ??
    ...
    let shiftMultiSelectAnchorElement: Element | null = null;
    let shiftMultiSelectAnchorPosition: Position | null = null;

    const clearShiftMultiSelectState = () => {
      shiftMultiSelectAnchorElement = null;
    ...
    const shouldTrackShiftMultiSelectPreview = () =>
      isShiftMultiSelectCommentMode() && store.frozenElements.length > 0;

    const updateShiftMultiSelection = (
      element: Element,
      position: Position,
    ) => {
      const nextSelectedElements = toggleElementSelection(
        store.frozenElements,
        element,
      );

      if (nextSelectedElements.length === 0) {
        clearShiftMultiSelectState();
        actions.unfreeze();
        return;
      }

      const nextAnchorElement =
        nextSelectedElements[nextSelectedElements.length - 1] ?? null;
      ...
      const nextAnchorPosition = nextSelectedElements.includes(element)
        ? position
        : getElementCenter(nextAnchorElement);

      shiftMultiSelectAnchorElement = nextAnchorElement;
      shiftMultiSelectAnchorPosition = nextAnchorPosition;

      freezeAllAnimations(nextSelectedElements);
      actions.setPointer(nextAnchorPosition);
      actions.setFrozenElements(nextSelectedElements);
      actions.freeze();
    };
    ...
    const notifyElementsSelected = async (
      ...
      window.dispatchEvent(
        new CustomEvent("ui-grab:element-selected", {
          detail: {
    ...
    const handleCopySuccessWithComments = (options: {
      ...
      if (!commentItemIdToUpdate && hasCopiedElements && extraPrompt) {
      ...
      let updatedCommentItems: CommentItem[];
      if (commentItemIdToUpdate) {
        updatedCommentItems = updateCommentItem(
          commentItemIdToUpdate,
          nextCommentItem,
        );
      } else {
        if (!extraPrompt) return;
        updatedCommentItems = addCommentItem(nextCommentItem);
      }

      setCommentItems(updatedCommentItems);
      setClockFlashTrigger((previous) => previous + 1);
      if (commentItemIdToUpdate && hasCopiedElements) {
        commentElementMap.set(commentItemIdToUpdate, [...copiedElements]);
      } else {
    ...
    const shiftMultiSelectPreviewElements = createMemo((): Element[] => {
      void store.viewportVersion;

      if (!shouldTrackShiftMultiSelectPreview()) return [];

      const candidate = targetElement();
      if (!candidate || isRootElement(candidate)) return [];
      if (store.frozenElements.includes(candidate)) return [];

      return toggleElementSelection(store.frozenElements, candidate).filter(
        (element): element is Element =>
          isElementConnected(element) && !isRootElement(element),
      );
    });

    const shiftMultiSelectPreviewBounds = createMemo((): OverlayBounds[] =>
      shiftMultiSelectPreviewElements().map((element) =>
    ...
    const selectionBounds = createMemo((): OverlayBounds | undefined => {
      ...
      const previewBounds = shiftMultiSelectPreviewBounds();
      if (previewBounds.length > 0) {
        if (previewBounds.length === 1) {
          const firstPreviewBounds = previewBounds[0];
          if (firstPreviewBounds) return firstPreviewBounds;
        }
        return createFlatOverlayBounds(combineBounds(previewBounds));
      }
    ...
    const selectionBoundsMultiple = createMemo((): OverlayBounds[] => {
      ...
      const shiftPreviewBounds = shiftMultiSelectPreviewBounds();
      if (shiftPreviewBounds.length > 0) {
        return shiftPreviewBounds;
      }
      return frozenElementsBounds();
    });

    const selectionElementsCount = createMemo(() => {
      const previewElements = shiftMultiSelectPreviewElements();
      if (previewElements.length > 0) {
        return previewElements.length;
      }
    ...
    const setCursorOverride = (cursor: string | null) => {
      if (cursor) {
        if (!cursorStyleElement) {
          ...
          cursorStyleElement.setAttribute("data-ui-grab-cursor", "");
    ...
    const handleInputSubmit = () => {
      ...
      const commentItemIdToUpdate = editingCommentItemId();
      ...
      performCopyWithLabel({
        ...
        commentItemIdToUpdate: commentItemIdToUpdate ?? undefined,
    ...
    const handlePointerMove = (clientX: number, clientY: number) => {
      if (
        !isEnabled() ||
        isPromptMode() ||
        (isFrozenPhase() && !shouldTrackShiftMultiSelectPreview()) ||
    ...
    const handlePointerDown = (
      ...
      const shouldPreserveFrozenSelection =
        isShiftKeyHeld &&
        isShiftMultiSelectCommentMode() &&
        store.frozenElements.length > 0;

      actions.startDrag(
        { x: clientX, y: clientY },
        shouldPreserveFrozenSelection,
      );
      actions.setPointer({ x: clientX, y: clientY });
    ...
    const handlePointerUp = (
      ...
      if (dragSelectionRect) {
        ...
        handleSingleClick(clientX, clientY, hasModifierKeyHeld, isShiftKeyHeld);
      }
    };
    ...
    eventListenerManager.addWindowListener(
      ...
      (event: KeyboardEvent) => {
        ...
        const isFromUiGrabInput = isEventFromOverlay(
          event,
          "data-ui-grab-input",
        );
        if (
        ...
        const isFromOverlay =
          isEventFromOverlay(event, "data-ui-grab-ignore-events") &&
    ...
    eventListenerManager.addWindowListener(
      ...
      (event: KeyboardEvent) => {
        ...
        if (event.key === "Shift") {
          ...
          if (
            isShiftMultiSelectCommentMode() &&
            shiftMultiSelectAnchorElement &&
            shiftMultiSelectAnchorPosition &&
            store.frozenElements.length > 0
          ) {
            enterCommentModeForElement(
              shiftMultiSelectAnchorElement,
              shiftMultiSelectAnchorPosition.x,
              shiftMultiSelectAnchorPosition.y,
            );
            return;
          }
        }
      ...
      (event: MouseEvent) => {
        ...
        if (isRendererActive() || isCopying() || didJustDrag()) {
          ...
          if (store.wasActivatedByToggle && !isCopying() && !isPromptMode()) {
            const shouldKeepShiftMultiSelection =
              event.shiftKey &&
              isShiftMultiSelectCommentMode() &&
              store.frozenElements.length > 0;
            if (!isHoldingKeys() && !shouldKeepShiftMultiSelection) {
              deactivateRenderer();
            } else {
    ...
    eventListenerManager.addWindowListener(
      "focusin",
      (event: FocusEvent) => {
        if (isEventFromOverlay(event, "data-ui-grab")) {
          event.stopPropagation();
        }
      },
      { capture: true },
    ...
    const selectionVisible = createMemo(() => {
      ...
      if (shiftMultiSelectPreviewBounds().length > 0) return true;
    ...
    const handleActivateCommentItem = (item: CommentItem) => {
    ...
    const handleCommentItemDelete = (item: CommentItem) => {
      clearCommentsHoverPreviews();
      commentElementMap.delete(item.id);

      if (activeCommentItemId() === item.id) {
        setActiveCommentItemId(null);
      }
      if (editingCommentItemId() === item.id) {
        setEditingCommentItemId(null);
        if (isPromptMode()) {
          actions.exitPromptMode();
          actions.clearInputText();
      ...
      const updatedCommentItems = removeCommentItem(item.id);
      setCommentItems(updatedCommentItems);

      if (updatedCommentItems.length === 0) {
        dismissCommentsDropdown();
      }
    };
    ...
    if (pluginRegistry.store.theme.enabled) {
      ...
      void import("../components/renderer.js")
          ...
          disposeRenderer = render(() => {
            return (
              <UiGrabRenderer
                ...
                selectionElementsCount={selectionElementsCount()}
        ...
        .catch((error) => {
          console.warn("[ui-grab] Failed to load renderer:", error);
        });
    }
    ...
    const api: UiGrabAPI = {
      ...
      setToolbarState: (state: Partial<ToolbarState>) => {
        ...
        const edge = state.edge ?? currentState?.edge ?? "bottom";
        const scale = resolveToolbarScale(
          {
            ...currentState,
            ...state,
